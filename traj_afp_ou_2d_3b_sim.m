function [exp_traj,exp_var,t_vec,num_sim]=traj_afp_ou_2d_3b_sim(ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,process_range,t_min,sel,stop_type,stop_val,runmed_width)
% Expected trajectory and expected variance (aligned
% with respect to the first passage) of a bounded
% (3 (time-variant) boundaries), (time-variant) 2D Ornstein-
% Uhlenbeck process. The calculation is based on a simulation.
%
% J. Ditterich, 7/09
%
% [exp_traj,exp_var,t_vec,num_sim] = traj_afp_ou_2d_3b_sim (ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,
...
if isnumeric(ou_drift)&&(length(ou_drift)~=2)
    error('TRAJ_AFP_OU_2D_3B_SIM: OU_DRIFT must be either a vector of length 2 or the name of a function!');
end;
...
if ou_leak<0
    error('TRAJ_AFP_OU_2D_3B_SIM: OU_LEAK must be a non-negative number!');
end;

if isnumeric(ou_cov)&&((size(ou_cov,1)~=2)||(size(ou_cov,2)~=2))
    error('TRAJ_AFP_OU_2D_3B_SIM: OU_COV must either be a 2-by-2 matrix or the name of a function!');
end;

if isnumeric(ou_cov)&&((ou_cov(1,1)<=0)||(ou_cov(2,2)<=0))
    error('TRAJ_AFP_OU_2D_3B_SIM: The main diagonal elements of OU_COV must be positive!');
end;

if isnumeric(ou_cov)&&(det(ou_cov)==0)
    error('TRAJ_AFP_OU_2D_3B_SIM: The covariance matrix must not be singular!');
end;

if isnumeric(ou_cov)&&((det(ou_cov)<0)||(ou_cov(1,2)~=ou_cov(2,1)))
    error('TRAJ_AFP_OU_2D_3B_SIM: Invalid covariance matrix!');
end;

if ~((size(ou_init,1)==1)&&(size(ou_init,2)==2))&&~((size(ou_init,1)==2)&&(size(ou_init,2)==1))
    error('TRAJ_AFP_OU_2D_3B_SIM: OU_INIT must be a vector of length 2!');
end;
...
if delta_t<=0
    error('TRAJ_AFP_OU_2D_3B_SIM: The time step must be a positive number!');
end;

if (size(process_range,1)~=2)||(size(process_range,2)~=2)
    if (size(process_range,1)~=1)||(size(process_range,2)~=1)||(process_range~=0)
        error('TRAJ_AFP_OU_2D_3B_SIM: PROCESS_RANGE must either be a 2-by-2 matrix or 0!');
    end;
end;
...
if limited_range
    if (diff(process_range(1,:))<0)||(diff(process_range(2,:))<0) % screwed up range?
        error('TRAJ_AFP_OU_2D_3B_SIM: Invalid range!');
    end;
end;

if limited_range
    if (ou_init(1)<process_range(1,1))||(ou_init(1)>process_range(1,2))||(ou_init(2)<process_range(2,1))||(ou_init(2)>process_range(2,2))
        error('TRAJ_AFP_OU_2D_3B_SIM: Initial value out of range!');
    end;
end;

if t_min<=0
    error('TRAJ_AFP_OU_2D_3B_SIM: T_MIN must be a positive number!');
end;

if (sel~=0)&&(sel~=1)&&(sel~=2)&&(sel~=3)
    error('TRAJ_AFP_OU_2D_3B_SIM: SEL must be either 0, 1, 2, or 3!');
end;

if (stop_type~=1)&&(stop_type~=2)
    error('TRAJ_AFP_OU_2D_3B_SIM: STOP_TYPE must be either 1 or 2!');
end;

if stop_val<=0
    error('TRAJ_AFP_OU_2D_3B_SIM: STOP_VAL must be a positive number!');
end;

if runmed_width<0
    error('TRAJ_AFP_OU_2D_3B_SIM: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&&(~mod(runmed_width,2))
    error('TRAJ_AFP_OU_2D_3B_SIM: RUNMED_WIDTH must be an odd number!');
end;
    ...
    elseif cov_const&&(ou_leak==0)&&(limited_range==0) % This case is not much more difficult ...
        ...
        for i=1:vec_length
            ...
            if ~((size(temp,1)==1)&&(size(temp,2)==2))&&~((size(temp,1)==2)&&(size(temp,2)==1))
                error('TRAJ_AFP_OU_2D_3B_SIM: The drift returned by a function must be a vector of length 2!');
            end;
        ...
        for i=1:vec_length
            ...
            if cov_const % time-invariant covariance matrix?
                ...
                if (size(temp,1)~=2)||(size(temp,2)~=2)
                    error('TRAJ_AFP_OU_2D_3B_SIM: The covariance matrix returned by a function must be a 2-by-2 matrix!');
                end;
                
                if (temp(1,1)<=0)||(temp(2,2)<=0)
                    error('TRAJ_AFP_OU_2D_3B_SIM: Algorithm stopped due to a non-positive variance!');
                end;
                
                if det(temp)==0
                    error('TRAJ_AFP_OU_2D_3B_SIM: Algorithm stopped due to a singular covariance matrix!');
                end;
                
                if (det(temp)<0)||(temp(1,2)~=temp(2,1))
                    error('TRAJ_AFP_OU_2D_3B_SIM: Algorithm stopped due to an invalid covariance matrix!');
                end;
