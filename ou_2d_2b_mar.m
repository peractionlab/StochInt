function [g_1,g_2,t_vec,ret_num_tr_states,g_oor]=ou_2d_2b_mar(ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,delta_t,covered_range,num_tr_states,stop_type,stop_val,oor_type,mapping_range,runmed_width)
% Markov chain approximation of the first passage time problem
% for a (time-variant) 2D Ornstein-Uhlenbeck process with 2
% (time-variant) boundaries. The calculation is based on a
% Random Walk with Gaussian Increments.
%
% J. Ditterich, 10/01
%
% [g_1,g_2,t_vec,ret_num_tr_states,g_oor] = ou_2d_2b_mar (ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,delta_t,
...
%       g_1 or g_2 and is only calculated if oor_type is set to 2. NaN is
...
%     overlap. If the algorithm detects that both boundaries have been crossed
%     in the same time step it assumes that in half of the cases the first boundary
%     has been crossed first and that in the other half the second boundary has
%     been crossed first.
...
%          a transition to a third absorbing state, which means that you have
...
% additional checks of the covariance matrix and values returned by functions added on 10/11/01
% updated on 4/18/02: transition matrix was always recalculated even if it was not necessary
...
if length(num_tr_states)==1 % scalar passed?
    if num_tr_states==0 % Should the number of states be determined automatically?
        ...
        disp(['OU_2D_2B_MAR: The automatically chosen number of transient states for the first dimension is ' num2str(num_tr_states(1)) '.']);
        disp(['OU_2D_2B_MAR: The automatically chosen number of transient states for the second dimension is ' num2str(num_tr_states(2)) '.']);
        
        if (num_tr_states(1)<9)|(num_tr_states(2)<9) % number of states too small?
            error('OU_2D_2B_MAR: The results would be likely to be inaccurate. Please choose a smaller DELTA_T!');
        end;   
    else % number of states was given by the user
        if length(num_tr_states)~=2 % wrong vector length?
            error('OU_2D_2B_MAR: The number of transient states must be either a vector of length 2 or the scalar 0!');
        end;
        ...
        if (num_tr_states(1)<0)|(num_tr_states(2)<0) % negative number of states?
            error('OU_2D_2B_MAR: There must be at least one transient state for both dimensions!');
        end;
        
        if (num_tr_states(1)<9)|(num_tr_states(2)<9)
            warning('OU_2D_2B_MAR: You should choose at least 9 transient states per dimension. Otherwise the results are very likely to be inaccurate!');
        end;   
    end;
end;
...
if isnumeric(ou_drift)&~((size(ou_drift,1)==1)&(size(ou_drift,2)==2))&~((size(ou_drift,1)==2)&(size(ou_drift,2)==1))
    error('OU_2D_2B_MAR: OU_DRIFT must be either a vector of length 2 or the name of a function!');
end;
...
if ou_leak<0
    error('OU_2D_2B_MAR: OU_LEAK must be a non-negative number!');
end;

if isnumeric(ou_cov)&((size(ou_cov,1)~=2)|(size(ou_cov,2)~=2))
    error('OU_2D_2B_MAR: OU_COV must either be a 2-by-2 matrix or the name of a function!');
end;

if isnumeric(ou_cov)&((ou_cov(1,1)<=0)|(ou_cov(2,2)<=0))
    error('OU_2D_2B_MAR: The main diagonal elements of OU_COV must be positive!');
end;

if isnumeric(ou_cov)&(det(ou_cov)==0)
    error('OU_2D_2B_MAR: The covariance matrix must not be singular!');
end;

if isnumeric(ou_cov)&((det(ou_cov)<0)|(ou_cov(1,2)~=ou_cov(2,1)))
    error('OU_2D_2B_MAR: Invalid covariance matrix!');
end;

if size(ou_init,2)~=2
    error('OU_2D_2B_MAR: OU_INIT must be a n-by-2 matrix!');
end;

if delta_t<=0
    error('OU_2D_2B_MAR: The time step must be a positive number!');
end;

if (size(covered_range,1)~=2)|(size(covered_range,2)~=2)
    error('OU_2D_2B_MAR: COVERED_RANGE must be a 2-by-2 matrix!');
end;

if (diff(covered_range(1,:))<0)|(diff(covered_range(2,:))<0) % screwed up range?
    error('OU_2D_2B_MAR: Invalid range!');
end;

if (stop_type~=1)&(stop_type~=2)
    error('OU_2D_2B_MAR: STOP_TYPE must be either 1 or 2!');
end;

if stop_val<=0
    error('OU_2D_2B_MAR: STOP_VAL must be a positive number!');
end;

if (oor_type~=1)&(oor_type~=2)
    error('OU_2D_2B_MAR: OOR_TYPE must be either 1 or 2!');
end;

if mapping_range<=0
    error('OU_2D_2B_MAR: MAPPING_RANGE must be a positive number!');
end;

if runmed_width<0
    error('OU_2D_2B_MAR: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&(~mod(runmed_width,2))
    error('OU_2D_2B_MAR: RUNMED_WIDTH must be an odd number!');
end;
...
if ~isempty(ind) % Are there initial values, which do not map to transient states?
    error('OU_2D_2B_MAR: All initial values must map to transient states!');
end;
...
while (1)
    ...
    if oor_type==1 % no third absorbing state
        if (((stop_type==1)&(t>stop_val))|((stop_type==2)&((1-(sum(g_1')+sum(g_2'))*delta_t)<stop_val))) % enough?
            break;
        end;
    else % third absorbing state
        if (((stop_type==1)&(t>stop_val))|((stop_type==2)&((1-(sum(g_1')+sum(g_2')+sum(g_oor'))*delta_t)<stop_val))) % enough?
            break;
        end;
    end;
    ...
    if nnz(a_1_cur)==0 % no transient states outside or on boundary?
        warning('OU_2D_2B_MAR: First boundary out of range!');
    end;
    ...
    if nnz(a_2_cur)==0 % no transient states outside or on boundary?
        warning('OU_2D_2B_MAR: Second boundary out of range!');
    end;
    ...
    if ~drift_const % Do we have to update the current drift?
        ...
        if ~((size(temp,1)==1)&(size(temp,2)==2))&~((size(temp,1)==2)&(size(temp,2)==1))
            error('OU_2D_2B_MAR: The drift returned by a function must be a vector of length 2!');
        end;
    ...
    if ~cov_const % Do we have to update the current variance?
        ...
        if (size(temp,1)~=2)|(size(temp,2)~=2)
            error('OU_2D_2B_MAR: The covariance matrix returned by a function must be a 2-by-2 matrix!');
        end;
        
        if (temp(1,1)<=0)|(temp(2,2)<=0)
            warning('OU_2D_2B_MAR: Algorithm stopped due to a non-positive variance!');
            break;
        end;
        
        if det(temp)==0
            warning('OU_2D_2B_MAR: Algorithm stopped due to a singular covariance matrix!');
        end;
        
        if (det(temp)<0)|(temp(1,2)~=temp(2,1))
            warning('OU_2D_2B_MAR: Algorithm stopped due to an invalid covariance matrix!');
        end;
    ...
    if nnz(a_1_cur~=last_a_1)|nnz(a_2_cur~=last_a_2)|sum(drift_cur~=last_drift)|sum(sum(cov_cur~=last_cov))
        last_a_1=a_1_cur;
        last_a_2=a_2_cur;
        ...
        if oor_type==1 % 2 absorbing states
            R=sparse(prod(num_tr_states),2); % These will be the transition vectors to the absorbing states.
        else % 3 absorbing states
            R=sparse(prod(num_tr_states),3);
        end;
            ...
            for row_ind=1:length(x2) % walk through the transition "block"
                for col_ind=1:length(x1)
                    ...
                    if ~to_state % out of range?
                        ...
                        if b1_crossed&~b2_crossed % first boundary will be crossed, but not second boundary
                            R(from_state,1)=R(from_state,1)+y(row_ind,col_ind); % update the probability of crossing the first boundary
                        elseif ~b1_crossed&b2_crossed % second boundary will be crossed, but no first boundary
                            R(from_state,2)=R(from_state,2)+y(row_ind,col_ind); % update the probability of crossing the second boundary
                        elseif b1_crossed&b2_crossed % both boundaries would be crossed
                            ...
                            if oor_type==2 % third absorbing state?
                                R(from_state,3)=R(from_state,3)+y(row_ind,col_ind); % update the probability of leaving the range without crossing a boundary
                            else % only 2 absorbing states
                        ...
                        if a_1_cur(to_state)&~a_2_cur(to_state) % first boundary will be crossed, but not second boundary
                            R(from_state,1)=R(from_state,1)+y(row_ind,col_ind); % update the probability of crossing the first boundary
                        elseif ~a_1_cur(to_state)&a_2_cur(to_state) % second boundary will be crossed, but not first boundary
                            R(from_state,2)=R(from_state,2)+y(row_ind,col_ind); % update the probability of crossing the second boundary
                        elseif a_1_cur(to_state)&a_2_cur(to_state) % both boundaries would be crossed
                            R(from_state,1)=R(from_state,1)+.5*y(row_ind,col_ind); % This is the critical case.
                            % I have decided to add half of the probability to the probability of crossing the first boundary
                            % and the other half to the probability of crossing the second boundary.
                            R(from_state,2)=R(from_state,2)+.5*y(row_ind,col_ind);
    ...
    if oor_type==2 % 3 absorbing states?
        g_oor=[g_oor current_probs(ou_init,3)/delta_t];
    end;
...
if runmed_width % filtering requested?
    ...
    if length(g_1(1,:))<runmed_width % inappropriate filter width?
        error('OU_2D_2B_MAR: Inappropriate filter width!');
    end;
