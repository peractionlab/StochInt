function [g_1,g_2,g_3,t_vec,sc]=ou_2d_3b_sim_sc_add_time(ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,add_time,delta_t,process_range,max_time,num_sim,runmed_width)
...
% second strongest alternative at a fixed time after threshold crossing, using the convention
...
% J. Ditterich, 3/20
%
% [g_1,g_2,g_3,t_vec,sc] = ou_2d_3b_sim_sc_add_time (ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,add_time,delta_t,
...
%     multiple boundaries in the same time step will be registered as a crossing of
%     the first boundary.
% add_time is the additional amount of time after the threshold crossing
%          before the accumulators are read out to determine the second choice.
...
% History:
% derived on 3/4/20 from ou_2d_3b_sim_sc
...
if isnumeric(ou_drift)&(length(ou_drift)~=2)
    error('OU_2D_3B_SIM_SC_ADD_TIME: OU_DRIFT must be either a vector of length 2 or the name of a function!');
end;
...
if ou_leak<0
    error('OU_2D_3B_SIM_SC_ADD_TIME: OU_LEAK must be a non-negative number!');
end;

if isnumeric(ou_cov)&((size(ou_cov,1)~=2)|(size(ou_cov,2)~=2))
    error('OU_2D_3B_SIM_SC_ADD_TIME: OU_COV must either be a 2-by-2 matrix or the name of a function!');
end;

if isnumeric(ou_cov)&((ou_cov(1,1)<=0)|(ou_cov(2,2)<=0))
    error('OU_2D_3B_SIM_SC_ADD_TIME: The main diagonal elements of OU_COV must be positive!');
end;

if isnumeric(ou_cov)&(det(ou_cov)==0)
    error('OU_2D_3B_SIM_SC_ADD_TIME: The covariance matrix must not be singular!');
end;

if isnumeric(ou_cov)&((det(ou_cov)<0)|(ou_cov(1,2)~=ou_cov(2,1)))
    error('OU_2D_3B_SIM_SC_ADD_TIME: Invalid covariance matrix!');
end;

if ~((size(ou_init,1)==1)&(size(ou_init,2)==2))&~((size(ou_init,1)==2)&(size(ou_init,2)==1))
    error('OU_2D_3B_SIM_SC_ADD_TIME: OU_INIT must be a vector of length 2!');
end;
...
if add_time<0
    error('OU_2D_3B_SIM_SC_ADD_TIME: ADD_TIME must not be negative!');
end;

if delta_t<=0
    error('OU_2D_3B_SIM_SC_ADD_TIME: The time step must be a positive number!');
end;

if (size(process_range,1)~=2)|(size(process_range,2)~=2)
    if (size(process_range,1)~=1)|(size(process_range,2)~=1)|(process_range~=0)
        error('OU_2D_3B_SIM_SC_ADD_TIME: PROCESS_RANGE must either be a 2-by-2 matrix or 0!');
    end;
end;
...
if limited_range
    if (diff(process_range(1,:))<0)|(diff(process_range(2,:))<0) % screwed up range?
        error('OU_2D_3B_SIM_SC_ADD_TIME: Invalid range!');
    end;
end;

if limited_range
    if (ou_init(1)<process_range(1,1))|(ou_init(1)>process_range(1,2))|(ou_init(2)<process_range(2,1))|(ou_init(2)>process_range(2,2))
        error('OU_2D_3B_SIM_SC_ADD_TIME: Initial value out of range!');
    end;
end;

if max_time<=0
    error('OU_2D_3B_SIM_SC_ADD_TIME: MAX_TIME must be a positive number!');
end;

if num_sim<1
    error('OU_2D_3B_SIM_SC_ADD_TIME: A minimum of 1 simulation is required!');
end;

if runmed_width<0
    error('OU_2D_3B_SIM_SC_ADD_TIME: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&(~mod(runmed_width,2))
    error('OU_2D_3B_SIM_SC_ADD_TIME: RUNMED_WIDTH must be an odd number!');
end;
...
sc=zeros(3,3);
add_samples=round(add_time/delta_t);
...
for k=1:num_sim
    ...
    elseif cov_const&(ou_leak==0)&(limited_range==0) % This case is not much more difficult ...
        ...
        for i=1:vec_length
            ...
            if ~((size(temp,1)==1)&(size(temp,2)==2))&~((size(temp,1)==2)&(size(temp,2)==1))
                error('OU_2D_3B_SIM_SC_ADD_TIME: The drift returned by a function must be a vector of length 2!');
            end;
        ...
        for i=1:vec_length
            ...
            if cov_const % time-invariant covariance matrix?
                ...
                if (size(temp,1)~=2)|(size(temp,2)~=2)
                    error('OU_2D_3B_SIM_SC_ADD_TIME: The covariance matrix returned by a function must be a 2-by-2 matrix!');
                end;
                
                if (temp(1,1)<=0)|(temp(2,2)<=0)
                    error('OU_2D_3B_SIM_SC_ADD_TIME: Algorithm stopped due to a non-positive variance!');
                end;
                
                if det(temp)==0
                    error('OU_2D_3B_SIM_SC_ADD_TIME: Algorithm stopped due to a singular covariance matrix!');
                end;
                
                if (det(temp)<0)|(temp(1,2)~=temp(2,1))
                    error('OU_2D_3B_SIM_SC_ADD_TIME: Algorithm stopped due to an invalid covariance matrix!');
                end;
            ...
            if ~boundary_crossed % no boundary crossed yet?
                if feval(a_1,cur_val,i*delta_t) % first boundary crossed?
                    g_1(i)=g_1(i)+1; % register boundary crossing
                    boundary_crossed=1;
                    samples_to_go=add_samples;
                    
                    if ~samples_to_go % no additional sampling needed?
                ...
                elseif feval(a_2,cur_val,i*delta_t) % second boundary crossed?
                ...
                elseif feval(a_3,cur_val,i*delta_t) % third boundary crossed?
            ...
            else % boundary has already been crossed
                samples_to_go=samples_to_go-1;
                
                if ~samples_to_go % time to read out the integrators for the second choice?
                    switch boundary_crossed % which boundary had been crossed?
                        case 1
                            % find second strongest alternative
                            temp_2=cur_val(2);
                            temp_3=-cur_val(1)-cur_val(2);
    ...
    if ~boundary_tested    
        ...
        for i=1:vec_length
            if ~boundary_crossed % no boundary crossing detected yet?
                ...
                elseif feval(a_2,cur_traj(:,i)',i*delta_t) % second boundary crossed?
                    g_2(i)=g_2(i)+1; % register boundary crossing
                    boundary_crossed=2;
                    samples_to_go=add_samples;
                ...
                elseif feval(a_3,cur_traj(:,i)',i*delta_t) % third boundary crossed?
                    g_3(i)=g_3(i)+1; % register boundary crossing
                    boundary_crossed=3;
                    samples_to_go=add_samples;
                ...
                if ~samples_to_go % time to read out the integrators to determine the second choice?
                    switch boundary_crossed % which boundary had been crossed?
                        case 1
                            % find second strongest alternative
                            temp_2=cur_traj(2,i);
