function [g_upper,g_lower,t_vec,ret_num_tr_states]=ou_1d_2b_mar(ou_drift,ou_leak,ou_var,ou_init,a_upper,a_lower,delta_t,covered_range,num_tr_states,stop_type,stop_val,mapping_range,runmed_width)
...
% J. Ditterich, 10/01
%
% [g_upper,g_lower,t_vec,ret_num_tr_states] = ou_1d_2b_mar (ou_drift,ou_leak,ou_var,ou_init,a_upper,a_lower,delta_t,
...
% updated on 10/4/01: additional checks before calling the running median filter
% updated on 10/5/01: change in the variance check
...
if num_tr_states==0 % Should the number of states be determined automatically?
    ...
    disp(['OU_1D_2B_MAR: The automatically chosen number of transient states is ' num2str(num_tr_states) '.']);
    
    if num_tr_states<9 % number of states too small?
        error('OU_1D_2B_MAR: The results would be likely to be inaccurate. Please choose a smaller DELTA_T!');
    end;   
else % number of states was given by the user
    if num_tr_states<0 % negative number of states?
        error('OU_1D_2B_MAR: There must be at least one transient state!');
    end;
    
    if num_tr_states<9
        warning('OU_1D_2B_MAR: You should choose at least 9 transient states. Otherwise the results are very likely to be inaccurate!');
    end;   
end;
...
% Some checks
if ou_leak<0
    error('OU_1D_2B_MAR: OU_LEAK must be a non-negative number!');
end;

if delta_t<=0
    error('OU_1D_2B_MAR: The time step must be a positive number!');
end;

if diff(covered_range)<0 % screwed up range?
    error('OU_1D_2B_MAR: Invalid range!');
end;

if (stop_type~=1)&(stop_type~=2)
    error('OU_1D_2B_MAR: STOP_TYPE must be either 1 or 2!');
end;

if stop_val<=0
    error('OU_1D_2B_MAR: STOP_VAL must be a positive number!');
end;

if mapping_range<=0
    error('OU_1D_2B_MAR: MAPPING_RANGE must be a positive number!');
end;

if runmed_width<0
    error('OU_1D_2B_MAR: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&(~mod(runmed_width,2))
    error('OU_1D_2B_MAR: RUNMED_WIDTH must be an odd number!');
end;
...
if isnumeric(a_lower) % Is the first boundary time-invariant?
    ...
    if (a_lower<covered_range(1))|(a_lower>covered_range(2)) % boundary out of range?
        warning('OU_1D_2B_MAR: Boundary out of range!');
    end;
...
if isnumeric(ou_var) % Is the variance time-invariant?
    ...
    if ou_var<=0
        error('OU_1D_2B_MAR: The variance must be a positive number!');
    end;   
...
if ~isempty(ind1)|~isempty(ind2) % Are there initial values, which map to absorbing states?
    error('OU_1D_2B_MAR: All initial values must map to transient states!');
end;
...
while (1)
    ...
    if (((stop_type==1)&(t>stop_val))|((stop_type==2)&((1-(sum(g_lower')+sum(g_upper'))*delta_t)<stop_val))) % enough?
        break;
    end;
    ...
    if a_upper_cur<=a_lower_cur % crossing boundaries?
        warning('OU_1D_2B_MAR: Algorithm stopped due to crossing boundaries!');
        break;
    end;
    ...
    if ~var_const % Do we have to update the current variance?
        ...
        if temp<=0
            warning('OU_1D_2B_MAR: Algorithm stopped due to a non-positive variance!');
            break;
        end;      
...
if (~a_lower_const|~a_upper_const)&(runmed_width>0) % Are there time-variant boundaries?
    ...
    if length(g_lower(1,:))<runmed_width % inappropriate filter width?
        error('OU_1D_2B_MAR: Inappropriate filter width!');
    end;
