function [g_1,g_2,g_3,t_vec]=ou_2d_3b_two_cross_sim(ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,process_range,max_time,num_sim,runmed_width,report_incomplete)
...
% based on a simulation. The first boundary crossing determines the choice,
% but the process does not terminate until a second boundary crossing,
% which determines the decision time.
%
% J. Ditterich, 11/18
%
% [g_1,g_2,g_3,t_vec] = ou_2d_3b_two_cross_sim (ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,
%                                               process_range,max_time,num_sim[,runmed_width[,report_incomplete]])
%
...
%     second argument. In case an additional argument needs to be passed to
%     the function, a_1 can also be a cell array with two elements. In this
%     case the first cell has to be the name of the function, the second
%     cell is passed as the third function argument (after location and
%     time).
...
% report_incomplete is an optional parameter, which determines how trials
%                   with no second threshold crossing yet when max_time is reached are reported.
%                   1 means that the first crossing is reported with a time
%                   of max_time. 0 means that no threshold crossing is
%                   reported. The default value is 1.

% History:
% derived on 11/21/18 from OU_2D_3B_SIM
...
if isnumeric(ou_drift)&(length(ou_drift)~=2)
    error('OU_2D_3B_TWO_CROSS_SIM: OU_DRIFT must be either a vector of length 2 or the name of a function!');
end;
...
if ou_leak<0
    error('OU_2D_3B_TWO_CROSS_SIM: OU_LEAK must be a non-negative number!');
end;

if isnumeric(ou_cov)&((size(ou_cov,1)~=2)|(size(ou_cov,2)~=2))
    error('OU_2D_3B_TWO_CROSS_SIM: OU_COV must either be a 2-by-2 matrix or the name of a function!');
end;

if isnumeric(ou_cov)&((ou_cov(1,1)<=0)|(ou_cov(2,2)<=0))
    error('OU_2D_3B_TWO_CROSS_SIM: The main diagonal elements of OU_COV must be positive!');
end;

if isnumeric(ou_cov)&(det(ou_cov)==0)
    error('OU_2D_3B_TWO_CROSS_SIM: The covariance matrix must not be singular!');
end;

if isnumeric(ou_cov)&((det(ou_cov)<0)|(ou_cov(1,2)~=ou_cov(2,1)))
    error('OU_2D_3B_TWO_CROSS_SIM: Invalid covariance matrix!');
end;

if ~((size(ou_init,1)==1)&(size(ou_init,2)==2))&~((size(ou_init,1)==2)&(size(ou_init,2)==1))
    error('OU_2D_3B_TWO_CROSS_SIM: OU_INIT must be a vector of length 2!');
end;
...
if (iscell(a_1))&&(length(a_1)~=2)
    error('OU_2D_3B_TWO_CROSS_SIM: A_1 must have 2 elements when given as a cell array!');
end;

if (iscell(a_2))&&(length(a_2)~=2)
    error('OU_2D_3B_TWO_CROSS_SIM: A_2 must have 2 elements when given as a cell array!');
end;

if (iscell(a_3))&&(length(a_3)~=2)
    error('OU_2D_3B_TWO_CROSS_SIM: A_3 must have 2 elements when given as a cell array!');
end;

if delta_t<=0
    error('OU_2D_3B_TWO_CROSS_SIM: The time step must be a positive number!');
end;

if (size(process_range,1)~=2)|(size(process_range,2)~=2)
    if (size(process_range,1)~=1)|(size(process_range,2)~=1)|(process_range~=0)
        error('OU_2D_3B_TWO_CROSS_SIM: PROCESS_RANGE must either be a 2-by-2 matrix or 0!');
    end;
end;
...
if limited_range
    if (diff(process_range(1,:))<0)|(diff(process_range(2,:))<0) % screwed up range?
        error('OU_2D_3B_TWO_CROSS_SIM: Invalid range!');
    end;
end;

if limited_range
    if (ou_init(1)<process_range(1,1))|(ou_init(1)>process_range(1,2))|(ou_init(2)<process_range(2,1))|(ou_init(2)>process_range(2,2))
        error('OU_2D_3B_TWO_CROSS_SIM: Initial value out of range!');
    end;
end;

if max_time<=0
    error('OU_2D_3B_TWO_CROSS_SIM: MAX_TIME must be a positive number!');
end;

if num_sim<1
    error('OU_2D_3B_TWO_CROSS_SIM: A minimum of 1 simulation is required!');
end;

if runmed_width<0
    error('OU_2D_3B_TWO_CROSS_SIM: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&(~mod(runmed_width,2))
    error('OU_2D_3B_TWO_CROSS_SIM: RUNMED_WIDTH must be an odd number!');
end;

if (report_incomplete~=0)&(report_incomplete~=1)
    error('OU_2D_3B_TWO_CROSS_SIM: REPORT_INCOMPLETE has to be either 0 or 1!');
end;
...
for k=1:num_sim
    ...
    elseif cov_const&(ou_leak==0)&(limited_range==0) % This case is not much more difficult ...
        ...
        for i=1:vec_length
            ...
            if ~((size(temp,1)==1)&(size(temp,2)==2))&~((size(temp,1)==2)&(size(temp,2)==1))
                error('OU_2D_3B_TWO_CROSS_SIM: The drift returned by a function must be a vector of length 2!');
            end;
        ...
        for i=1:vec_length
            ...
            if cov_const % time-invariant covariance matrix?
                ...
                if (size(temp,1)~=2)|(size(temp,2)~=2)
                    error('OU_2D_3B_TWO_CROSS_SIM: The covariance matrix returned by a function must be a 2-by-2 matrix!');
                end;
                
                if (temp(1,1)<=0)|(temp(2,2)<=0)
                    error('OU_2D_3B_TWO_CROSS_SIM: Algorithm stopped due to a non-positive variance!');
                end;
                
                if det(temp)==0
                    error('OU_2D_3B_TWO_CROSS_SIM: Algorithm stopped due to a singular covariance matrix!');
                end;
                
                if (det(temp)<0)|(temp(1,2)~=temp(2,1))
                    error('OU_2D_3B_TWO_CROSS_SIM: Algorithm stopped due to an invalid covariance matrix!');
                end;
            ...
            if iscell(a_1) % Do we need to pass a third function argument?
                cross_test=feval(a_1{1},cur_val,i*delta_t,a_1{2});
            else % no third function argument
                cross_test=feval(a_1,cur_val,i*delta_t);
            end;
            
            if cross_test % first boundary crossed?
                if ~first_crossing % no boundary crossing detected yet?
                    first_crossing=1;
                elseif first_crossing~=1 % second boundary crossing with boundary being different from first one?
                    switch first_crossing
                        case 2
                            g_2(i)=g_2(i)+1; % register crossing of the first boundary with the time determined by the second boundary
                            second_crossing=1;
                        case 3
                            g_3(i)=g_3(i)+1;
                            second_crossing=1;
            ...
            if iscell(a_2) % Do we need to pass a third function argument?
                cross_test=feval(a_2{1},cur_val,i*delta_t,a_2{2});
            else % no third function argument
                cross_test=feval(a_2,cur_val,i*delta_t);
            end;
            
            if cross_test % second boundary crossed?
                if ~first_crossing % no boundary crossing detected yet?
                    first_crossing=2;
                elseif first_crossing~=2 % second boundary crossing with boundary being different from first one?
                    switch first_crossing
                        case 1
                            g_1(i)=g_1(i)+1; % register crossing of the first boundary with the time determined by the second boundary
                            second_crossing=1;
                        case 3
                            g_3(i)=g_3(i)+1;
                            second_crossing=1;
            ...
            if iscell(a_3) % Do we need to pass a third function argument?
                cross_test=feval(a_3{1},cur_val,i*delta_t,a_3{2});
            else % no third function argument
                cross_test=feval(a_3,cur_val,i*delta_t);
            end;
            
            if cross_test % third boundary crossed?
                if ~first_crossing % no boundary crossing detected yet?
                    first_crossing=3;
                elseif first_crossing~=3 % second boundary crossing with boundary being different from first one?
                    switch first_crossing
                        case 1
                            g_1(i)=g_1(i)+1; % register crossing of the first boundary with the time determined by the second boundary
                            second_crossing=1;
        ...
        if first_crossing&&(~second_crossing)&&report_incomplete % no second crossing detected
            switch first_crossing
                case 1
                    g_1(vec_length)=g_1(vec_length)+1; % register longest possible decision time
                case 2
                    g_2(vec_length)=g_2(vec_length)+1; % register longest possible decision time
                case 3
                    g_3(vec_length)=g_3(vec_length)+1; % register longest possible decision time
            end;
        end;
    end;
    ...
    if ~boundary_tested    
        for i=1:vec_length
            if iscell(a_1) % Do we need to pass a third function argument?
                cross_test=feval(a_1{1},cur_traj(:,i)',i*delta_t,a_1{2});
            else % no third function argument
                cross_test=feval(a_1,cur_traj(:,i)',i*delta_t);
            end;
            ...
            if iscell(a_2) % Do we need to pass a third function argument?
                cross_test=feval(a_2{1},cur_traj(:,i)',i*delta_t,a_2{2});
            else % no third function argument
                cross_test=feval(a_2,cur_traj(:,i)',i*delta_t);
            end;
            ...
            if iscell(a_3) % Do we need to pass a third function argument?
                cross_test=feval(a_3{1},cur_traj(:,i)',i*delta_t,a_3{2});
            else % no third function argument
                cross_test=feval(a_3,cur_traj(:,i)',i*delta_t);
            end;
