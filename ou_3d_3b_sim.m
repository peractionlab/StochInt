function [g_1,g_2,g_3,t_vec]=ou_3d_3b_sim(ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,process_range,max_time,num_sim,runmed_width)
% Estimate of the first passage time densities for a (time-variant)
% 3D Ornstein-Uhlenbeck process with 3 (time-variant) boundaries
% based on a simulation.
%
% J. Ditterich, 12/08
%
% [g_1,g_2,g_3,t_vec] = ou_3d_3b_sim (ou_drift,ou_leak,ou_cov,ou_init,a_1,a_2,a_3,delta_t,
...
%     second argument. In case an additional argument needs to be passed to the function,
%     a_1 can also be a cell array with two elements. In this case the first cell has
%     to be the name of the function, the second cell is passed as the third
%     function argument (after the time).
...
if isnumeric(ou_drift)&&(length(ou_drift)~=3)
    error('OU_3D_3B_SIM: OU_DRIFT must be either a vector of length 3 or the name of a function!');
end;
...
if ou_leak<0
    error('OU_3D_3B_SIM: OU_LEAK must be a non-negative number!');
end;

if isnumeric(ou_cov)&&((size(ou_cov,1)~=3)||(size(ou_cov,2)~=3))
    error('OU_3D_3B_SIM: OU_COV must either be a 3-by-3 matrix or the name of a function!');
end;

if isnumeric(ou_cov)&&((ou_cov(1,1)<=0)||(ou_cov(2,2)<=0)||(ou_cov(3,3)<=0))
    error('OU_3D_3B_SIM: The main diagonal elements of OU_COV must be positive!');
end;

if isnumeric(ou_cov)&&(det(ou_cov)==0)
    error('OU_3D_3B_SIM: The covariance matrix must not be singular!');
end;

if isnumeric(ou_cov)&&((det(ou_cov)<0)||(ou_cov(1,2)~=ou_cov(2,1))||(ou_cov(1,3)~=ou_cov(3,1))||(ou_cov(2,3)~=ou_cov(3,2)))
    error('OU_3D_3B_SIM: Invalid covariance matrix!');
end;

if ~((size(ou_init,1)==1)&&(size(ou_init,2)==3))&&~((size(ou_init,1)==3)&&(size(ou_init,2)==1))
    error('OU_3D_3B_SIM: OU_INIT must be a vector of length 3!');
end;
...
if (iscell(a_1))&&(length(a_1)~=2)
    error('OU_3D_3B_SIM: A_1 must have 2 elements when given as a cell array!');
end;

if (iscell(a_2))&&(length(a_2)~=2)
    error('OU_3D_3B_SIM: A_2 must have 2 elements when given as a cell array!');
end;

if (iscell(a_3))&&(length(a_3)~=2)
    error('OU_3D_3B_SIM: A_3 must have 2 elements when given as a cell array!');
end;

if delta_t<=0
    error('OU_3D_3B_SIM: The time step must be a positive number!');
end;

if (size(process_range,1)~=3)||(size(process_range,2)~=2)
    if (size(process_range,1)~=1)||(size(process_range,2)~=1)||(process_range~=0)
        error('OU_3D_3B_SIM: PROCESS_RANGE must either be a 3-by-2 matrix or 0!');
    end;
end;
...
if limited_range
    if (diff(process_range(1,:))<0)||(diff(process_range(2,:))<0)||(diff(process_range(3,:))<0) % screwed up range?
        error('OU_3D_3B_SIM: Invalid range!');
    end;
end;
...
if limited_range
    if (ou_init(1)<process_range(1,1))||(ou_init(1)>process_range(1,2))||(ou_init(2)<process_range(2,1))||(ou_init(2)>process_range(2,2))||(ou_init(3)<process_range(3,1))||(ou_init(3)>process_range(3,2))
        error('OU_3D_3B_SIM: Initial value out of range!');
    end;
end;

if max_time<=0
    error('OU_3D_3B_SIM: MAX_TIME must be a positive number!');
end;

if num_sim<1
    error('OU_3D_3B_SIM: A minimum of 1 simulation is required!');
end;

if runmed_width<0
    error('OU_3D_3B_SIM: RUNMED_WIDTH must not be negative!');
end;

if runmed_width&&(~mod(runmed_width,2))
    error('OU_3D_3B_SIM: RUNMED_WIDTH must be an odd number!');
end;
...
for k=1:num_sim
    ...
    elseif cov_const&&(ou_leak==0)&&(limited_range==0) % This case is not much more difficult ...
        ...
        for i=1:vec_length
            ...
            if ~((size(temp,1)==1)&&(size(temp,2)==3))&&~((size(temp,1)==3)&&(size(temp,2)==1))
                error('OU_3D_3B_SIM: The drift returned by a function must be a vector of length 3!');
            end;
        ...
        for i=1:vec_length
            ...
            if cov_const % time-invariant covariance matrix?
                ...
                if (size(temp,1)~=3)||(size(temp,2)~=3)
                    error('OU_3D_3B_SIM: The covariance matrix returned by a function must be a 3-by-3 matrix!');
                end;
                
                if (temp(1,1)<=0)||(temp(2,2)<=0)||(temp(3,3)<=0)
                    error('OU_3D_3B_SIM: Algorithm stopped due to a non-positive variance!');
                end;
                
                if det(temp)==0
                    error('OU_3D_3B_SIM: Algorithm stopped due to a singular covariance matrix!');
                end;
                
                if (det(temp)<0)||(temp(1,2)~=temp(2,1))||(temp(1,3)~=temp(3,1))||(temp(2,3)~=temp(3,2))
                    error('OU_3D_3B_SIM: Algorithm stopped due to an invalid covariance matrix!');
                end;
            ...
            if iscell(a_1) % Do we need to pass an additional function argument?
                if feval(a_1{1},cur_val,i*delta_t,a_1{2}) % first boundary crossed?
                    g_1(i)=g_1(i)+1; % register boundary crossing
                    break; % We no longer have to calculate the rest of the trajectory.
                end;
            else % no additional function argument
            ...
            if iscell(a_2) % Do we need to pass an additional function argument?
                if feval(a_2{1},cur_val,i*delta_t,a_2{2}) % second boundary crossed?
            ...
            if iscell(a_3) % Do we need to pass an additional function argument?
                if feval(a_3{1},cur_val,i*delta_t,a_3{2}) % third boundary crossed?
    ...
    if ~boundary_tested    
        for i=1:vec_length
            if iscell(a_1) % Do we need to pass an additional function argument?
                if feval(a_1{1},cur_traj(:,i)',i*delta_t,a_1{2}) % first boundary crossed?
                    g_1(i)=g_1(i)+1; % register boundary crossing
                    break;
                end;
            else % no additional function argument
            ...
            if iscell(a_2) % Do we need to pass an additional function argument?
                if feval(a_2{1},cur_traj(:,i)',i*delta_t,a_2{2}) % second boundary crossed?
                    g_2(i)=g_2(i)+1; % register boundary crossing
                    break;
                end;
            else % no additional function argument
            ...
            if iscell(a_3) % Do we need to pass an additional function argument?
                if feval(a_3{1},cur_traj(:,i)',i*delta_t,a_3{2}) % third boundary crossed?
                    g_3(i)=g_3(i)+1; % register boundary crossing
                    break;
                end;
            else % no additional function argument
