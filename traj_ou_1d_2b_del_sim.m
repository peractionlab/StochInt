function [exp_traj,exp_var,t_vec,num_sim]=traj_ou_1d_2b_del_sim(ou_drift,ou_leak,ou_var,ou_init,a_upper,a_lower,del_1_mean,del_1_sd,del_2_mean,del_2_sd,delta_t,t_min,sel,stop_type,stop_val,runmed_width,align_timevar)
...
% J. Ditterich, 7/02
%
% [exp_traj,exp_var,t_vec,num_sim] = traj_ou_1d_2b_del_sim (ou_drift,ou_leak,ou_var,ou_init,a_upper,a_lower,
...
% Some checks
if ou_leak<0
    error('TRAJ_OU_1D_2B_DEL_SIM: OU_LEAK must be a non-negative number!');
end;

if (size(ou_init,1)~=1)|(size(ou_init,2)~=1)
    error('TRAJ_OU_1D_2B_DEL_SIM: OU_INIT must be a scalar!');
end;

if del_1_mean<0
    error('TRAJ_OU_1D_2B_DEL_SIM: DEL_1_MEAN must not be negative!');
end;

if del_1_sd<0
    error('TRAJ_OU_1D_2B_DEL_SIM: DEL_1_SD must not be negative!');
end;

if del_2_mean<0
    error('TRAJ_OU_1D_2B_DEL_SIM: DEL_2_MEAN must not be negative!');
end;

if del_2_sd<0
    error('TRAJ_OU_1D_2B_DEL_SIM: DEL_2_SD must not be negative!');
end;

if delta_t<=0
    error('TRAJ_OU_1D_2B_DEL_SIM: The time step must be a positive number!');
end;

if t_min<=0
    error('TRAJ_OU_1D_2B_DEL_SIM: T_MIN must be a positive number!');
end;

if (sel~=0)&(sel~=1)&(sel~=2)
    error('TRAJ_OU_1D_2B_DEL_SIM: SEL must be either 0, 1, or 2!');
end;

if (stop_type~=1)&(stop_type~=2)
    error('TRAJ_OU_1D_2B_DEL_SIM: STOP_TYPE must be either 1 or 2!');
end;

if stop_val<=0
    error('TRAJ_OU_1D_2B_DEL_SIM: STOP_VAL must be a positive number!');
end;

if runmed_width<0
    error('TRAJ_OU_1D_2B_DEL_SIM: RUNMED_WIDTH must not be negative!');
end;

if (runmed_width>0)&(~mod(runmed_width,2))
    error('TRAJ_OU_1D_2B_DEL_SIM: RUNMED_WIDTH must be an odd number!');
end;

if (align_timevar~=1)&(align_timevar~=2)
    error('TRAJ_OU_1D_2B_DEL_SIM: ALIGN_TIMEVAR must be either 1 or 2!');
end;
...
if isnumeric(a_lower) % Is the first boundary time-invariant?
    ...
    if ou_init<=a_lower
        error('TRAJ_OU_1D_2B_DEL_SIM: The initial value is out of range!');
    end;
else
    a_lower_const=0;
end;
...
if isnumeric(ou_var) % Is the variance time-invariant?
    ...
    if ou_var<=0
        error('TRAJ_OU_1D_2B_DEL_SIM: The variance must be a positive number!');
    end;
...
if runmed_width>vec_length
    error('TRAJ_OU_1D_2B_DEL_SIM: Inappropriate filter width!');
end;
        ...
        for i=1:vec_length
            if i>del_1_dis % initial delay?
                ...
                if var_const % time-invariant variance?
                    ...
                    if temp<=0
                        error('TRAJ_OU_1D_2B_DEL_SIM: The variance must be positive!');
                    end;
