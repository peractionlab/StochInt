function [exp_traj,exp_var,t_vec,num_sim]=traj_ou_vd_1d_2b_sim(ou_drift_mean,ou_drift_sd,ou_leak,ou_var,ou_init,a_upper,a_lower,delta_t,t_min,sel,stop_type,stop_val,runmed_width)
...
% J. Ditterich, 10/01
%
% [exp_traj,exp_var,t_vec,num_sim] = traj_ou_vd_1d_2b_sim (ou_drift_mean,ou_drift_sd,ou_leak,ou_var,ou_init,a_upper,a_lower,
...
if ~isnumeric(ou_drift_mean)
    error('TRAJ_OU_VD_1D_2B_SIM: OU_DRIFT_MEAN must be a number!');
end;

if ou_drift_sd<=0
    error('TRAJ_OU_VD_1D_2B_SIM: OU_DRIFT_SD must be a positive number!');
end;

if ou_leak<0
    error('TRAJ_OU_VD_1D_2B_SIM: OU_LEAK must be a non-negative number!');
end;

if (size(ou_init,1)~=1)|(size(ou_init,2)~=1)
    error('TRAJ_OU_VD_1D_2B_SIM: OU_INIT must be a scalar!');
end;

if delta_t<=0
    error('TRAJ_OU_VD_1D_2B_SIM: The time step must be a positive number!');
end;

if t_min<=0
    error('TRAJ_OU_VD_1D_2B_SIM: T_MIN must be a positive number!');
end;

if (sel~=0)&(sel~=1)&(sel~=2)
    error('TRAJ_OU_VD_1D_2B_SIM: SEL must be either 0, 1, or 2!');
end;

if (stop_type~=1)&(stop_type~=2)
    error('TRAJ_OU_VD_1D_2B_SIM: STOP_TYPE must be either 1 or 2!');
end;

if stop_val<=0
    error('TRAJ_OU_VD_1D_2B_SIM: STOP_VAL must be a positive number!');
end;

if runmed_width<0
    error('TRAJ_OU_VD_1D_2B_SIM: RUNMED_WIDTH must not be negative!');
end;

if (runmed_width>0)&(~mod(runmed_width,2))
    error('TRAJ_OU_VD_1D_2B_SIM: RUNMED_WIDTH must be an odd number!');
end;
...
if isnumeric(a_lower) % Is the first boundary time-invariant?
    ...
    if ou_init<=a_lower
        error('TRAJ_OU_VD_1D_2B_SIM: The initial value is out of range!');
    end;
else
    a_lower_const=0;
end;
...
if isnumeric(ou_var) % Is the variance time-invariant?
    ...
    if ou_var<=0
        error('TRAJ_OU_VD_1D_2B_SIM: The variance must be a positive number!');
    end;
...
if runmed_width>vec_length
    error('TRAJ_OU_VD_1D_2B_SIM: Inappropriate filter width!');
end;
    ...
    if var_const&(ou_leak==0) % In this case we can do it in a single step.
        ...
        for i=1:vec_length
            ...
            if var_const % time-invariant variance?
                ...
                if temp<=0
                    error('TRAJ_OU_VD_1D_2B_SIM: The variance must be positive!');
                end;
